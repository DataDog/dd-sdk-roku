...
sub initialize(configuration as object, global as object)
    ...
    if (datadogUploader = invalid)
        ' Build the task but do NOT start it yet: only the instance that wins the shared
        ' `datadogUploader` slot starts its loop, so a concurrent or repeated initialize()
        ' leaves inert nodes that never start (and are garbage-collected) instead of extra running
        ' uploader threads racing on (and re-uploading / failing to delete) the same batch files.
    ...
    if (configuration.applicationId = invalid or configuration.applicationId = "")
        ...
        ' Build the agent but do NOT start it yet: its setup() (which spawns the WriterTask,
        ' scopes and crash reporter, and registers its track) runs only when the task starts,
        ' so only the winner of the shared slot creates them; a loser stays inert and is GC'd.
        ...
        if (global.datadogRumAgent = invalid)
            ' Publish the RUM context (observed synchronously by early logs) and the agent
            ' together, so the registered instanceId always matches the agent that runs.
            global.addFields({
                datadogRumContext: {
                    applicationId: configuration.applicationId
                    instanceId: rumInstanceId
        ...
        if (rumAgent.isSameNode(global.datadogRumAgent))
            ddLogInfo("No RUM agent, creating one")
            global.datadogRumAgent.control = "RUN"
        else
    ...
    if (global.datadogLogsAgent = invalid)
        ...
        ' Publish only if we won the slot. A loser is never referenced by callers (they use
        ' global.datadogLogsAgent), so it lazily creates no WriterTask and is garbage-collected.
        if (global.datadogLogsAgent = invalid)
            global.addFields({
                datadogLogsAgent: agent
            })
        end if
        if (agent.isSameNode(global.datadogLogsAgent))
