...
sub handleResponse(responseCode as integer, filePath as string)
    if (responseCode = 202)
        ddLogInfo("Upload to track " + m.top.trackType + " succeeded.")
        retry = false
    else if (responseCode = 400)
        ddLogError("Upload to track " + m.top.trackType + " failed, data will be discarded;")
        ...
        ddLogWarning("Upload to track " + m.top.trackType + " timed out, we'll retry later")
        retry = true
        ...
        ddLogWarning("Upload to track " + m.top.trackType + " failed, we'll retry later;")
        ...
        ddLogWarning("Upload to track " + m.top.trackType + " failed, data will be discarded;")
        ddLogWarning("The intake server returned status code " + responseCode.toStr() + "; if the issue persits, please contact the support.")
        retry = false
    else
        ddLogWarning("Upload to track " + m.top.trackType + " status unknown;")
        ddLogWarning("The intake server returned status code " + responseCode.toStr() + "; if the issue persits, please contact the support.")
        retry = false
    end if
    if (not retry)
