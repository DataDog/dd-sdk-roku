...
function getUploadUrl(trackInfo as object) as string
    url = "https://" + trackInfo.endpointHost + "/api/v2/" + trackInfo.trackType
...
' ----------------------------------------------------------------
function uploadFile(path as string, trackInfo as object) as integer
    ensureNetworkClient()
...
' @param trackInfo (object) the configuration for the current track
' ----------------------------------------------------------------
sub handleResponse(responseCode as integer, filePath as string, trackInfo as object)
    if (responseCode = 202)
