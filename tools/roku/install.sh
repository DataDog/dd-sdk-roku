...
HTTP_STATUS=`curl --keepalive-time 600 --limit-rate 500K --user "$ROKU_DEV_USERNAME:$ROKU_DEV_PASSWORD" --digest --silent --show-error -F "mysubmit=Delete" -F "archive=" --output $DEV_SERVER_TMP_FILE --write-out "%{http_code}" http://$ROKU_DEV_TARGET/plugin_install`
result=$?
if (( $result > 0 )); then
...
HTTP_STATUS=`curl --keepalive-time 600 --limit-rate 500K --user "$ROKU_DEV_USERNAME:$ROKU_DEV_PASSWORD" --digest --silent --show-error -F "mysubmit=Install" -F "archive=@$1" --output $DEV_SERVER_TMP_FILE --write-out "%{http_code}" -m 20 http://$ROKU_DEV_TARGET/plugin_install`
result=$?
if (( $result > 0 )); then
    echo "ERROR: Device '$ROKU_DEV_TARGET_NAME' ($ROKU_DEV_TARGET) is not responding.";
    exit $result
